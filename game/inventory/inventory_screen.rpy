        ...
        grid 6 6:
            ...
            for item in inventory.get_page_items(inventory.current_page):
                if item:
                    vbox:
                        if item.sprite:
                            imagebutton idle item.sprite action [SetVariable("selected_item", item), ShowMenu("item_options", inventory=inventory)] hovered Show("item_info", item=item) unhovered Hide("item_info") at resize_30x30
...
screen item_options(inventory):
    frame:
        ...
        textbutton "Discard" action [Hide("option_text"), ShowMenu("discard_item", inventory=inventory)] style "inventory_button"  # Change font color to black

screen discard_item(inventory):
    frame:
        xalign 0.5
        yalign 0.5
        background None  # Make the frame background transparent
        ...
        hbox:
            textbutton "Discard" action [Function(discard_selected_item, discard_quantity), Hide("discard_item"), Hide("item_options"), Show("inventory_screen", inventory=inventory)] style "inventory_button"  # Change font color to black
            textbutton "Cancel" action [Hide("discard_item"), Show("item_options", inventory=inventory)] style "inventory_button"  # Change font color to black
