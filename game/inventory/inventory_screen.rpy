        ...
        grid 6 6:
            ...
            for item in inventory.get_page_items(inventory.current_page):
                if item:
                    vbox:
                        if item.sprite:
                            imagebutton idle item.sprite hover item.sprite action [SetVariable("selected_item", item), Show("item_options")] hovered [SetVariable("hovered_item", item), Show("item_info", item=item)] unhovered [Hide("item_info"), SetVariable("hovered_item", None)] at resize_30x30
...
screen discard_item():
    ...
    frame:
        ...
        background "#888C"
        has vbox
        text "How many [selected_item.name] would you like to discard?" size 20 style "item_options"
